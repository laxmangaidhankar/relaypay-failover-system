...
const { moveFunds, reconcileWalletBalance } = require('../services/ledgerService');
...
async function transitionState( transaction, toStatus, {actorId = null, metadata = {} } = {}){
  const fromStatus = transaction.status;
  const allowed = ALLOWED_TRANSITIONS[fromStatus] || [];


  if(!allowed.includes(toStatus)){
    throw new Error(`Illegal transition: ${fromStatus} -> ${toStatus} for transaction ${transaction._id}`);
  }
...
async function initiatePayment({ senderId, senderWalletId, receiverId, receiverWalletId, amount, idempotencyKey, note }) {
  ...
  await transitionState(transaction, 'PRIMARY_ATTEMPTED', { actorId: senderId });
   ...
   try {
    ...
    notificationService.notifyTransactionSettled(senderId, {
      transactionId: transaction._id, amount, newBalance: result.fromBalance,
    });
...
async function handlePrimaryFailure(transaction, err) {
  const failureReason = err instanceof BankError ? err.code : 'BANK_SERVER_ERROR';
  ...
  if (!isRelayEligible) {
    ...
    notificationService.notifyTransactionFailed(transaction.senderId, {
      transactionId: transaction._id, reason: failureReason,
    });
    return transaction;
  }
...
async function triggerRelay(transaction) {
  const backup = await relayService.findBackupContact(transaction.senderId, transaction.amount);
 
  if (!backup) {
    await transitionState(transaction, 'REVERSED', { metadata: { reason: 'no_eligible_backup_contact' } });
    notificationService.notifyTransactionFailed(transaction.senderId, {
      transactionId: transaction._id, reason: 'NO_BACKUP_AVAILABLE',
    });
    return transaction;
  }
 
  const timeoutSeconds = await relayService.getRelayTimeoutSeconds(transaction.senderId);
...
async function handleRelayResponse(transactionId, responderId, decision) {
  ...
  if (transaction.status !== 'RELAY_REQUESTED') {
    throw new Error(`Transaction ${transactionId} is not awaiting relay response (status: ${transaction.status})`);
  }
  if (String(transaction.relay.backupContactId) !== String(responderId)) {
    throw new Error('Only the invited backup contact may respond to this relay request');
  }
  ...
  if (decision === 'DECLINE') {
    await transaction.save();
    await transitionState(transaction, 'RELAY_DECLINED', { actorId: responderId });
    await transitionState(transaction, 'REVERSED', { actorId: responderId, metadata: { reason: 'relay_declined' } });
    notificationService.notifyRelayResolved(transaction.senderId, {
      transactionId: transaction._id, status: 'DECLINED', amount: transaction.amount,
    });
    return transaction;
  }
  ...
  await transaction.save();
  await transitionState(transaction, 'RELAY_APPROVED', { actorId: responderId });
  ...
  await relayService.recordRelayUsage(transaction.senderId, responderId, transaction.amount);
 
  notificationService.notifyRelayResolved(transaction.senderId, {
    transactionId: transaction._id, status: 'APPROVED', amount: transaction.amount,
  });
  notificationService.notifyTransactionSettled(transaction.senderId, {
    transactionId: transaction._id, amount: transaction.amount, newBalance: null,
  });
...
async function handleRelayTimeout(transactionId) {
  ...
  await transitionState(transaction, 'REVERSED', { metadata: { reason: 'relay_timeout' } });
 
  notificationService.notifyTransactionFailed(transaction.senderId, {
    transactionId: transaction._id, reason: 'RELAY_TIMEOUT',
  });
